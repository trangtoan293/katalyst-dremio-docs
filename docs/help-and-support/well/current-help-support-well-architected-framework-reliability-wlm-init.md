...
## Configure Workload Management for a New installations[​](/help-support/well-architected-framework/reliability/wlm-init/#configure-workload-management-for-a-new-installations "Direct link to Configure Workload Management for a New installations")
...
### No Engines or Single Engine Configuration[​](/help-support/well-architected-framework/reliability/wlm-init/#no-engines-or-single-engine-configuration "Direct link to No Engines or Single Engine Configuration")
...
![](https://docs.dremio.com/assets/images/wlm-init-3e3c21a17ee83a219b81a148bd29c19f.png)
...
#### Average Node Memory = 16384 (16 GB)[​](/help-support/well-architected-framework/reliability/wlm-init/#average-node-memory--16384-16-gb "Direct link to Average Node Memory = 16384 \(16 GB\)")  
| Queue Name  | Max Concurrency Limit  | Queue Memory Limit per Node  | Job Memory Limit per Node  |  
| --- | --- | --- | --- |  
...
#### Average Node Memory = 32768 (32 GB)[​](/help-support/well-architected-framework/reliability/wlm-init/#average-node-memory--32768--32-gb "Direct link to Average Node Memory = 32768  \(32 GB\)")  
| Queue Name  | Max Concurrency Limit  | Queue Memory Limit per Node  | Job Memory Limit per Node  |  
| --- | --- | --- | --- |  
...
#### Average Node Memory = 53248 (52 GB)[​](/help-support/well-architected-framework/reliability/wlm-init/#average-node-memory--53248-52-gb "Direct link to Average Node Memory = 53248 \(52 GB\)")  
| Queue Name  | Max Concurrency Limit  | Queue Memory Limit per Node  | Job Memory Limit per Node  |  
| --- | --- | --- | --- |  
...
#### Average Node Memory = 114688 (112 GB)[​](/help-support/well-architected-framework/reliability/wlm-init/#average-node-memory--114688-112-gb "Direct link to Average Node Memory = 114688 \(112 GB\)")  
| Queue Name  | Max Concurrency Limit  | Queue Memory Limit per Node  | Job Memory Limit per Node  |  
| --- | --- | --- | --- |  
...
### Multi-Engine Configuration (Kubernetes)[​](/help-support/well-architected-framework/reliability/wlm-init/#multi-engine-configuration-kubernetes "Direct link to Multi-Engine Configuration \(Kubernetes\)")
...
## Configuring Queues Programmatically[​](/help-support/well-architected-framework/reliability/wlm-init/#configuring-queues-programmatically "Direct link to Configuring Queues Programmatically")
A newly installed Dremio cluster comes with five queues with default configurations. Dremio provides APIs to update memory configuration of queues. An approach to programmatically configure queues, based on cluster size, is provided below.
  * Dremio needs the auth token to make any API call. Auth token can be obtained using the API `/apiv2/login POST` method. More details can be found here: [Dremio Login](/reference/api#authentication)
  * Next, you should calculate the average direct memory by engine by querying Dremio system tables `sys.nodes` and `sys.memory`. You can submit the query via [API](/reference/api/sql#submit-an-sql-query) or JDBC.
  ...
  * For each queue, Dremio generates a unique Queue ID. Get the current queue configuration using `GET /api/v3/wlm/queue`. Additional details can be found in [Dremio All Queues](/reference/api/wlm/queue#retrieve-all-queues)
  * For each queue, update Dremio with the modified queue configuration using the API `PUT /api/v3/wlm/queue/{id}`. Additional details can be found in [Update Queue](/reference/api/wlm/queue#update-a-queue).
