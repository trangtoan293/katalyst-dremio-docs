...
## Supported Drivers[​](/client-applications#supported-drivers "Direct link to Supported Drivers")
...
## Developing Custom Applications[​](/client-applications#developing-custom-applications "Direct link to Developing Custom Applications")
...
## Client Encryption[​](/client-applications#client-encryption "Direct link to Client Encryption")
Transport Layer Security (TLS) communication is supported for encrypting communication between legacy JDBC client applications and Dremio servers. See the configuration of client TLS for [Dremio on Kubernetes](/deploy-dremio/configuring-kubernetes#transport-level-security) or [Dremio standalone clusters](/deploy-dremio/other-options/standalone/dremio-config/dremio-conf/wire-encryption-config#legacy-jdbc-clients-enterprise) for more information.
Transport Layer Security (TLS) communication is enabled by default for Arrow Flight client applications. See the configuration of Arrow Flight encryption for [Dremio on Kubernetes](/deploy-dremio/configuring-kubernetes#transport-level-security) or [Dremio standalone clusters](/deploy-dremio/other-options/standalone/dremio-config/dremio-conf/wire-encryption-config#arrow-flight-and-arrow-flight-sql-jdbc-and-odbc-enterprise) for more information. If you want to connect via unencrypted connections, you must explicitly disable `useEncryption` by setting it to `false` in the [connection parameters](/client-applications/drivers/arrow-flight-sql-odbc-driver#connection-parameters) for the Arrow Flight SQL ODBC driver.
