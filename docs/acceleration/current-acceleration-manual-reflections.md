...
slug: /acceleration/manual-reflections
...
For creating Reflections on views and tables with row-access and column-masking policies, see [Using Reflections on Datasets with Policies](/data-products/govern/row-column-policies-udf).
### Prerequisites​
...
See [External Reflections](/acceleration) for a description of what external Reflections are and their benefits.
...
Dremio does not allow expiration policies to be set on external Reflections or Reflections that automatically refresh when Iceberg data changes according to the [refresh policy](/acceleration/manual-reflections/refreshing-reflections).
