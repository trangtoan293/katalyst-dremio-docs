...
slug: /security/integrations/lake-formation
...
When Lake Formation is properly configured, Dremio adheres to the following workflow each time an end user attempts to access, edit, or query datasets with managed privileges:
  1. Dremio enforces [access control](/security/rbac). See Configuring Sources for Lake Formation below for access control recommendations.
...
Use [support keys](/help-support/support-settings/) to disable the cache or customize the cache time-to-live (TTL):
