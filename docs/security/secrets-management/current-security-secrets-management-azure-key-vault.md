...
After you complete the [prerequisites](/security/secrets-management/azure-key-vault#prerequisites), you can create secrets in Azure Key Vault and use them in Dremio. Dremio uses the [secret references](/security/secrets-management/azure-key-vault#connecting-to-data-sources) you provide to retrieve secret values at runtime and authenticate to [data sources](/security/secrets-management/azure-key-vault#connecting-to-data-sources) and [other services](/security/secrets-management/azure-key-vault#dremio-configuration-files).
## Supported Data Sources[​](/security/secrets-management/azure-key-vault#supported-data-sources "Direct link to Supported Data Sources")
...
## Supported Configuration Files and Properties[​](/security/secrets-management/azure-key-vault#supported-configuration-files-and-properties "Direct link to Supported Configuration Files and Properties")
...
## Prerequisites[​](/security/secrets-management/azure-key-vault#prerequisites "Direct link to Prerequisites")
Dremio uses Azure 
![](https://docs.dremio.com/assets/images/managed-identity-client-id-75d69f53a9c8329c58e71f9c3c91d29d.png) **Example:** The Client ID of the managed identity is shown on the right as `d91a5b36-189d-4137-bf1f-7c0a36ef11b0`. 
...
When deploying Dremio on a Kubernetes cluster in the **Azure Kubernetes Service (AKS):**
  ...
  3. [Configure an environment variable](/deploy-dremio/configuring-kubernetes#configuring-extra-environment-variables) for your Azure managed identity Client ID. Add the `AZURE_MANAGED_IDENTITY_CLIENT_ID` environment variable to your `values-overrides.yaml` for the `coordinator` and `executor` with the value of your Azure managed identity Client ID.
...
When deploying Dremio on **Azure virtual machines (VMs)** :
  ...
  2. Provide access to Azure Key Vault using 
  3. Edit the Dremio [dremio-env](/deploy-dremio/other-options/standalone/dremio-config/dremio-env) file on the coordinator and each executor to add the `AZURE_MANAGED_IDENTITY_CLIENT_ID` environment variable with the Client ID of your managed identity. Export this variable definition from `dremio-env`.
...
## Retrieving the Secret Reference from Azure Key Vault[​](/security/secrets-management/azure-key-vault#retrieving-the-secret-reference-from-azure-key-vault "Direct link to Retrieving the Secret Reference from Azure Key Vault")
...
![](https://docs.dremio.com/assets/images/azure-key-vault-secret-identifier-location-9f19514d49256d10d3a972418d6ea890.png) **Example:** The Azure Key Vault secret value for use in Dremio is `https://sourcecreds.vault.azure.net/secrets/azurestorage`. 
## Using A Secret Reference[​](/security/secrets-management/azure-key-vault#using-a-secret-reference "Direct link to Using A Secret Reference")
### Connecting to Data Sources[​](/security/secrets-management/azure-key-vault#connecting-to-data-sources "Direct link to Connecting to Data Sources")
...
### Dremio Configuration Files[​](/security/secrets-management/azure-key-vault#dremio-configuration-files "Direct link to Dremio Configuration Files")
...
### Secrets Rotation[​](/security/secrets-management/azure-key-vault#secrets-rotation "Direct link to Secrets Rotation")
