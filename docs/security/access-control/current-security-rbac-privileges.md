...
Privileges that are inheritable also implicitly apply to child objects through [inheritance](/security/rbac).
...
| EXECUTE  | 
 ...
 |  
| EXPORT DIAGNOSTICS  |  [Download the cluster logs](/admin/monitoring/) using the Dremio console.  |  
| EXTERNAL QUERY  | Run [external queries](/reference/sql/commands) on the sources in the system.
...
| EXTERNAL QUERY  | Run [external queries](/reference/sql/commands) on the source.
  * This privilege is only supported for Amazon Redshift, Microsoft SQL Server, MySQL, and PostgreSQL sources and Dremio Hub connectors that use advanced relational pushdown (ARP).
...
| OWNERSHIP  | The privileges provided by ownership depend on your configuration. 
  ...
  * In [managed access spaces](/security/rbac), the owner of a folder cannot grant and revoke privileges. 
...
## User-Defined Function (UDF) Privileges​ Privileges")  
| PRIVILEGE  | DESCRIPTION  |  
| --- | --- |  
...
| OWNERSHIP  | The privileges provided by ownership depend on your configuration. 
  ...
  * In [managed access spaces](/security/rbac), the owner of a UDF cannot grant and revoke privileges on the UDF. 
...
| OWNERSHIP  | The privileges provided by ownership depend on your configuration. 
  ...
  * In [managed access spaces](/security/rbac), the owner of a view cannot grant and revoke privileges on the view. 
...
| PRIVILEGE  | DESCRIPTION  |  
| --- | --- |  
| ALTER  | Set a new password for the user and change the user's [type](/security/authentication/users) from local (internal) to external.
