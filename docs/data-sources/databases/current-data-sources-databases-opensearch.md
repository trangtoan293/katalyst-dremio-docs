...
slug: /data-sources/databases/opensearch
...
Choose one of the following authentication methods:
  ...
  * **AWS Profile** : Dremio sources profile credentials from the specified AWS profile. For information on how to set up a configuration or credentials file for AWS, see [AWS Custom Authentication](/data-sources/object/s3). Under AWS Profile (Optional), enter the AWS profile name. If this is left blank, then the default profile will be used. For more information about using profiles in a credentials or configuration file, see AWS's documentation on 
