...
## Prerequisites[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#prerequisites "Direct link to Prerequisites")
Before you can manually run the VACUUM CATALOG command, you must complete the following tasks:
  1. [Add the Nessie catalog as a source](/data-sources/lakehouse-catalogs/nessie#configuring-nessie-as-a-source) in Dremio.
  2. [Set the cutoff policy](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#setting-the-cutoff-policy) for the Nessie catalog source.


You must be the catalog owner or a member of the ADMIN role to manually run the VACUUM CATALOG command, and the vacuum operation is only supported for Nessie sources. Read [Configuring Nessie as a Source](/data-sources/lakehouse-catalogs/nessie#configuring-nessie-as-a-source) for more information.
## Setting the Cutoff Policy[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#setting-the-cutoff-policy "Direct link to Setting the Cutoff Policy")
Dremio uses the cutoff policy for the Nessie catalog source to determine which snapshots and associated files to expire and delete. For example, if the cutoff policy is set to 5 days, Dremio expires and deletes snapshots that are older than 5 days. For details about how Dremio executes cutoff policies, see the [VACUUM CATALOG examples](/reference/sql/commands/nessie/vacuum-catalog#examples).
Dremio applies a 5-day cutoff policy for table cleanup by default. Use the [duration-based](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#setting-a-duration-based-cutoff-policy) or [time-based](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#setting-a-time-based-cutoff-policy) cutoff policy, with or without a grace period.
...
#### Parameters[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#parameters "Direct link to Parameters")
[config](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#config-parameters) Body Object
Container for the configuration parameters for the cutoff policy.
#####  `config` Parameters[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#config-parameters "Direct link to config-parameters")
type Body String
...
### Setting a Duration-Based Cutoff Policy[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#setting-a-duration-based-cutoff-policy "Direct link to Setting a Duration-Based Cutoff Policy")
With a [duration-based cutoff policy](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#setting-the-cutoff-policy), Dremio deletes snapshots that are older than the specified duration each time you run the VACUUM CATALOG command.
...
### Setting a Time-Based Cutoff Policy[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#setting-a-time-based-cutoff-policy "Direct link to Setting a Time-Based Cutoff Policy")
With a [time-based cutoff policy](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#setting-the-cutoff-policy), Dremio deletes snapshots that are older than the specified date and time.
...
### Updating the Cutoff Policy[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#updating-the-cutoff-policy "Direct link to Updating the Cutoff Policy")
To update the cutoff policy for a Nessie catalog source, send a new request to the same POST endpoint you use to [set the cutoff policy](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#setting-the-cutoff-policy). If you previously set a grace period for the Nessie catalog source, the update request body must include the `newFilesGracePeriod` parameter and the duration you want to use, even if you don't want to change the grace period.
...
### Removing the Cutoff Policy[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#removing-the-cutoff-policy "Direct link to Removing the Cutoff Policy")
...
## Overrides and Exclusions[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#overrides-and-exclusions "Direct link to Overrides and Exclusions")
...
## Results and Output[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#results-and-output "Direct link to Results and Output")
When the VACUUM CATALOG command runs, Dremio removes expired snapshots and orphaned metadata files for tables in the catalog if the following conditions are met:
  * The snapshot is older than the number of days specified in the [defaultCutoffPolicy parameter](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#setting-the-cutoff-policy).
...
## Common Scenarios[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#common-scenarios "Direct link to Common Scenarios")
...
### Snapshots Older than the Cutoff Policy[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#snapshots-older-than-the-cutoff-policy "Direct link to Snapshots Older than the Cutoff Policy")
...
### Branch Older than the Cutoff Policy[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#branch-older-than-the-cutoff-policy "Direct link to Branch Older than the Cutoff Policy")
...
### Commit Created After the Cutoff Policy[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#commit-created-after-the-cutoff-policy "Direct link to Commit Created After the Cutoff Policy")
...
## Viewing Table Cleanup Jobs[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#viewing-table-cleanup-jobs "Direct link to Viewing Table Cleanup Jobs")
...
## Limitations[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#limitations "Direct link to Limitations")
...
## Troubleshooting Table Cleanup Jobs[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#troubleshooting-table-cleanup-jobs "Direct link to Troubleshooting Table Cleanup Jobs")
...
### Validation Errors[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#validation-errors "Direct link to Validation Errors")
...
To resolve these errors, [update the cutoff policy](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#updating-the-cutoff-policy) for the Nessie catalog source.
### File Not Found Exception[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#file-not-found-exception "Direct link to File Not Found Exception")
...
### Vacuum Runs but Does Not Delete Any Files[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#vacuum-runs-but-does-not-delete-any-files "Direct link to Vacuum Runs but Does Not Delete Any Files")
...
### Vacuum Runs but Does Not Delete Expired Files[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#vacuum-runs-but-does-not-delete-expired-files "Direct link to Vacuum Runs but Does Not Delete Expired Files")
...
### Vacuum Does Not Run[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#vacuum-does-not-run "Direct link to Vacuum Does Not Run")
...
### Missing Intermediate Files[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#missing-intermediate-files "Direct link to Missing Intermediate Files")
...
### Error While Expiring Snapshots[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#error-while-expiring-snapshots "Direct link to Error While Expiring Snapshots")
The 
### Long-Running Vacuum[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#long-running-vacuum "Direct link to Long-Running Vacuum")
...
### Number of Files Increases after VACUUM CATALOG Runs[​](/developer/data-formats/apache-iceberg/table-maintenance-optimization/table-cleanup-vacuum#number-of-files-increases-after-vacuum-catalog-runs "Direct link to Number of Files Increases after VACUUM CATALOG Runs")
...
[Previous Automated Maintenance with the Open Catalog](/developer/data-formats/apache-iceberg/table-maintenance-optimization/automated-maintenance)
![](https://cdn.bizible.com/ipv?_biz_r=&_biz_h=800054037&_biz_u=6cd305d62a4c402de07902b3246ffbbc&_biz_l=https%3A%2F%2Fdocs.dremio.com%2Fcurrent%2Fdeveloper%2Fdata-formats%2Fapache-iceberg%2Ftable-maintenance-optimization%2Ftable-cleanup-vacuum%2F&_biz_t=1777951098384&_biz_i=Table%20Cleanup%20with%20Vacuum%20%7C%20Dremio%20Documentation&_biz_n=1520&rnd=376742&cdn_o=a&_biz_z=1777951098385)
