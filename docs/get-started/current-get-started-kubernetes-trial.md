...
slug: /get-started/kubernetes-trial
...
Let's start by deploying the Enterprise Edition on your hosted Kubernetes environment:
  ...
  3. Open the `values-overrides.yaml` file in an editor to make the following configurations:
    1. For distributed storage, follow the instructions in [Configuring the Distributed Storage](/deploy-dremio/configuring-kubernetes), and then return here.
    2. For object storage, follow the instructions in [Configuring Storage for the Open Catalog](/deploy-dremio/configuring-kubernetes), and then return here.
    3. (Optional) For the coordinator, you can adjust its default values by following the instructions in [Recommended Resources Configuration](/deploy-dremio/configuring-kubernetes), and then return here.
