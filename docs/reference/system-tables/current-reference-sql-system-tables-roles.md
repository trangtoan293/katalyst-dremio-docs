...
slug: /reference/sql/system-tables/roles
...
Valid values:  
`LOCAL`: A role [created and managed within Dremio](/security/rbac/roles)  
`EXTERNAL`: A role created and managed by an [external authentication service](/security/rbac/roles) like Okta or Microsoft Entra ID  |  
