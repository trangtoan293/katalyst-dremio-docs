...
Any user can create personal access tokens (PATs) [in the Dremio console](/security/authentication/personal-access-tokens) or [using REST](/reference/api/personal-access-token). Users can configure the lifetime of each personal access token, from 1 day to a maximum defined by the `auth.personal-access-token.max_lifetime_days` [support setting](/help-support/support-settings/).
...
As an example, the [Reflection summary](/reference/api/reflections/reflection-summary) endpoint supports the `pageToken` parameter. If the Reflection summary contains more than 50 results, the response will include the `nextPageToken` attribute. To retrieve the next 50 results, add `?pageToken={nextPageToken_value}` to the request URL:
...
Catalog API endpoints for retrieving non-filesystem [sources](/reference/api/catalog/source), [spaces](/reference/api/catalog/container-space), and [folders](/reference/api/catalog/container-folder) by ID or path support the `maxChildren` query parameter for specifying the maximum number of child objects to include in each response. If the response contains more than the specified number of child objects, the response includes the `nextPageToken` attribute. To retrieve the next page of results, add `?pageToken={nextPageToken_value}` to the request URL. This example shows a request URL that uses the `nextPageToken` query parameter with the `maxChildren` query parameter set to 25:
...
As an example, the [Reflection summary](/reference/api/reflections/reflection-summary) endpoint supports the `filter` query parameter for certain specific attributes. To retrieve only the raw Reflections that are refreshed manually or by schedule, are enabled, and apply to datasets with `samples.dremio.com` in their paths, the `filter` JSON object would look like this:
...
For example, the [Reflection summary](/reference/api/reflections/reflection-summary) endpoint supports ordering the response by ReflectionName, datasetName, or reflectionType. To organize the response in ascending order by ReflectionName:
