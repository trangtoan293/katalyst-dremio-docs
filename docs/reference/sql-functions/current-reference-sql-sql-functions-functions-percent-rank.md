...
slug: /reference/sql/sql-functions/functions/PERCENT_RANK
...
## Syntax
### PERCENT_RANK() OVER ( [PARTITION BY partition_expression] [ORDER BY order_expression]) → double[​](/reference/sql/sql-functions) OVER \( \[PARTITION BY partition_expression\] \[ORDER BY order_expression\]\) → double")
