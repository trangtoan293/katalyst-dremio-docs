...
## Object Hierarchy[​](/security/rbac#object-hierarchy "Direct link to Object Hierarchy")
...
![](https://docs.dremio.com/assets/images/rbac-inheritance-5ad539f979430ed0b6b949a9d4a50830.png)
...
![](https://docs.dremio.com/assets/images/rbac-scope-55174fe1a171329a6f9194c775e27f77.png)
Because of the established scope, `user1` may not access `FolderD` because they were only granted access to `FolderC`'s objects.
### Current vs. Future Objects[​](/security/rbac#current-vs-future-objects "Direct link to Current vs. Future Objects")
Based on the selected scope, you may restrict a user's access to future and existing datasets. For example, if you select a single table as the scope of a user's privilege, then that user may only perform that action to the existing dataset, as well as any future views they create using that table. However, they may not access any views created from a table by another user ([see the example below](/security/rbac#example-single-dataset)). However, if the scope is instead set at the folder level, then the user may perform the granted privilege to all tables and views contained in that folder ([see the example below](/security/rbac#example-scope)).
## Ownership[​](/security/rbac#ownership "Direct link to Ownership")
...
The following behaviors and limitations apply to ownership:
  ...
  * The object's access control settings may not work if the owner is deleted or removed. See [View Delegation](/security/rbac#view-delegation).
...
### Managed Access Spaces[​](/security/rbac#managed-access-spaces "Direct link to Managed Access Spaces")
...
When using managed access spaces, Dremio displays shared spaces with a lock icon ![](https://docs.dremio.com/images/icons/managed-access-icon.png) on the [Datasets](/get-started/quick_tour/#datasets-page) and [SQL Runner](/get-started/quick_tour/#sql-runner) pages. Owners of folders, views, and functions in a locked space cannot grant or revoke privileges on those objects to other users or roles.  
...
The Dremio administrator can activate managed access spaces by setting the `security.access-control.managed-access-spaces.enabled` [support key](/help-support/support-settings/#support-keys) on the Support Settings page.
### View Delegation[​](/security/rbac#view-delegation "Direct link to View Delegation")
...
#### Example 1: View Delegation[​](/security/rbac#example-1-view-delegation "Direct link to Example 1: View Delegation")
`user1` has the SELECT privilege on `table1` and creates `view1` to filter and transform data in `table1`. `user2` asks for access privileges to run queries on `view1` as well. `user2` may obtain the SELECT privilege for `view1` from the following authorized users:
  ...
  * A limited set of users and roles, such as the space owner, can grant or revoke privileges in [managed access spaces](/security/rbac#managed-access-spaces).
...
#### Example 2: View Delegation with Revoked Access to the Original Table[​](/security/rbac#example-2-view-delegation-with-revoked-access-to-the-original-table "Direct link to Example 2: View Delegation with Revoked Access to the Original Table")
...
## Privileges[​](/security/rbac#privileges "Direct link to Privileges")
...
### Granting Privileges Using the Dremio Console[​](/security/rbac#granting-privileges-using-the-dremio-console "Direct link to Granting Privileges Using the Dremio Console")
...
![](https://docs.dremio.com/assets/images/privileges-users-table-5eb046e1d2acd0cc1a6d1bd87d3afc35.png)
  1. (Optional) Repeat steps 4-6 if you want to add more users or roles and grant them privileges.
  2. When finished, click **Save**.


### Revoking Privileges Using the Dremio Console[​](/security/rbac#revoking-privileges-using-the-dremio-console "Direct link to Revoking Privileges Using the Dremio Console")
...
### Granting Privileges Using SQL Commands[​](/security/rbac#granting-privileges-using-sql-commands "Direct link to Granting Privileges Using SQL Commands")
When granting privileges to users and roles with SQL commands, you may follow one of three methods: [granting to a single dataset](/security/rbac#granting-to-a-single-dataset), [granting to ALL DATASETS](/security/rbac#granting-to-all-datasets), and [granting to a scope](/security/rbac#granting-to-a-scope). Examples of these methods may be found under each section.
...
Because all users are members of the PUBLIC role, you can use the PUBLIC role to grant privileges to all users.
#### Granting to a Single Dataset[​](/security/rbac#granting-to-a-single-dataset "Direct link to Granting to a Single Dataset")
...
##### Example: Single Dataset[​](/security/rbac#example-single-dataset "Direct link to Example: Single Dataset")
...
![](https://docs.dremio.com/assets/images/rbac-privilege-1-ab4d9d59f7940dddc05d90ba62556db9.png)
...
#### Granting to ALL DATASETS[​](/security/rbac#granting-to-all-datasets "Direct link to Granting to ALL DATASETS")
...
##### Example: All Datasets[​](/security/rbac#example-all-datasets "Direct link to Example: All Datasets")
...
The image below illustrates the objects `user1` now has access to.
![](https://docs.dremio.com/assets/images/rbac-privilege-2-22c128dbaec6b244cca0f812b40f7d06.png)
...
#### Granting to a Scope[​](/security/rbac#granting-to-a-scope "Direct link to Granting to a Scope")
...
##### Example: Scope[​](/security/rbac#example-scope "Direct link to Example: Scope")
...
The image below illustrates the objects `user1` now has access to.
![](https://docs.dremio.com/assets/images/rbac-privilege-3-121f99249e16dfe376040d3c4cae2b40.png)
...
[Previous Personal Access Tokens](/security/authentication/personal-access-tokens)[Next Privileges](/security/rbac/privileges)
![](https://cdn.bizible.com/ipv?_biz_r=&_biz_h=800054037&_biz_u=6cd305d62a4c402de07902b3246ffbbc&_biz_l=https%3A%2F%2Fdocs.dremio.com%2Fcurrent%2Fsecurity%2Frbac%2F&_biz_t=1777950400043&_biz_i=Access%20Control%20%7C%20Dremio%20Documentation&_biz_n=161&rnd=82049&cdn_o=a&_biz_z=1777950400043)
