---
url: /reference/api
title: "API Reference | Dremio Enterprise Documentation"
depth: 1
crawled_at: 64004.487196291
---
...
## Base URL[​](/reference/api#base-url "Direct link to Base URL")
...
## Authentication[​](/reference/api#authentication "Direct link to Authentication")
Each REST request requires an authorization header with a Dremio access token to authenticate the requester unless otherwise indicated. Dremio accepts three types of access tokens for authenticating REST requests.
  * **[OAuth access tokens](/reference/api#oauth-access-tokens-enterprise)** are created in Dremio using the Dremio REST API.
  * **[Personal access tokens](/reference/api#personal-access-tokens-enterprise)** are created in the Dremio console or with REST.
  * **[Authentication tokens](/reference/api#dremio-authentication-tokens)** are generated from a username and password using the Dremio v2 API.
...
### OAuth Access Tokens Enterprise[​](/reference/api#oauth-access-tokens-enterprise "Direct link to oauth-access-tokens-enterprise")
Users can create OAuth access tokens by exchanging a local or LDAP username and password, a PAT, or an external JWT using the `/oauth/token` REST API. Dremio provides [sample code](/reference/api/oauth-token) for each of these cases.
...
### Personal Access Tokens Enterprise[​](/reference/api#personal-access-tokens-enterprise "Direct link to personal-access-tokens-enterprise")
Any user can create personal access tokens (PATs) [in the Dremio console](/security/authentication/personal-access-tokens#creating-a-pat) or [using REST](/reference/api/personal-access-token#create-a-pat). Users can configure the lifetime of each personal access token, from 1 day to a maximum defined by the `auth.personal-access-token.max_lifetime_days` [support setting](/help-support/support-settings/#support-keys).
### Dremio Authentication Tokens[​](/reference/api#dremio-authentication-tokens "Direct link to Dremio Authentication Tokens")
...
## Errors[​](/reference/api#errors "Direct link to Errors")
...
## Query Parameters[​](/reference/api#query-parameters "Direct link to Query Parameters")
...
As an example, the [Reflection summary](/reference/api/reflections/reflection-summary#retrieve-a-reflection-summary) endpoint supports the `pageToken` parameter. If the Reflection summary contains more than 50 results, the response will include the `nextPageToken` attribute. To retrieve the next 50 results, add `?pageToken={nextPageToken_value}` to the request URL:
...
#### User-Specified Maximum[​](/reference/api#user-specified-maximum "Direct link to User-Specified Maximum")
...
Catalog API endpoints for retrieving non-filesystem [sources](/reference/api/catalog/source#retrieve-a-source-by-id), [spaces](/reference/api/catalog/container-space#retrieve-a-space-by-id), and [folders](/reference/api/catalog/container-folder#retrieve-a-folder-by-id) by ID or path support the [`maxChildren` query parameter](/reference/api#maxchildren-query-parameter) for specifying the maximum number of child objects to include in each response. If the response contains more than the specified number of child objects, the response includes the `nextPageToken` attribute. To retrieve the next page of results, add `?pageToken={nextPageToken_value}` to the request URL. This example shows a request URL that uses the `nextPageToken` query parameter with the `maxChildren` query parameter set to 25:
...
For subsequent requests, replace the `pageToken` value in the request URL with the token value from the previous response. If the response does not include a token attribute and value, you have retrieved the last page of available results.
### maxResults Query Parameter[​](/reference/api#maxresults-query-parameter "Direct link to maxResults Query Parameter")
...
### filter Query Parameter[​](/reference/api#filter-query-parameter "Direct link to filter Query Parameter")
...
As an example, the [Reflection summary](/reference/api/reflections/reflection-summary#retrieve-a-reflection-summary) endpoint supports the `filter` query parameter for certain specific attributes. To retrieve only the raw Reflections that are refreshed manually or by schedule, are enabled, and apply to datasets with `samples.dremio.com` in their paths, the `filter` JSON object would look like this:
...
### orderBy Query Parameter[​](/reference/api#orderby-query-parameter "Direct link to orderBy Query Parameter")
...
For example, the [Reflection summary](/reference/api/reflections/reflection-summary#retrieve-a-reflection-summary) endpoint supports ordering the response by ReflectionName, datasetName, or reflectionType. To organize the response in ascending order by ReflectionName:
...
### limit and offset Query Parameters[​](/reference/api#limit-and-offset-query-parameters "Direct link to limit and offset Query Parameters")
...
### type Query Parameter[​](/reference/api#type-query-parameter "Direct link to type Query Parameter")
...
### include and exclude Query Parameters[​](/reference/api#include-and-exclude-query-parameters "Direct link to include and exclude Query Parameters")
...
### createdBy Query Parameter[​](/reference/api#createdby-query-parameter "Direct link to createdBy Query Parameter")
...
### ownedBy Query Parameter[​](/reference/api#ownedby-query-parameter "Direct link to ownedBy Query Parameter")
...
### search Query Parameter[​](/reference/api#search-query-parameter "Direct link to search Query Parameter")
...
### maxChildren Query Parameter[​](/reference/api#maxchildren-query-parameter "Direct link to maxChildren Query Parameter")
...
Use the `maxChildren` query parameter in concert with the [`pageToken` query parameter](/reference/api#user-specified-maximum) to split large sets of results into multiple pages.
## Response Headers[​](/reference/api#response-headers "Direct link to Response Headers")
...
[Previous Admin CLI](/reference/admin-cli)[Next Catalog](/reference/api/catalog)
...
[Previous Admin CLI](/reference/admin-cli)[Next Catalog](/reference/api/catalog)
![](https://cdn.bizible.com/ipv?_biz_r=&_biz_h=800054037&_biz_u=6cd305d62a4c402de07902b3246ffbbc&_biz_l=https%3A%2F%2Fdocs.dremio.com%2Fcurrent%2Freference%2Fapi%2F&_biz_t=1777950325499&_biz_i=API%20Reference%20%7C%20Dremio%20Documentation&_biz_n=15&rnd=243924&cdn_o=a&_biz_z=1777950325499)
